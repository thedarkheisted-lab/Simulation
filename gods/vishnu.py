...
class Vishnu:
    ...
    def influence_battle(self, c1, c2):
        ...
        if target.health < 0.5 * target.max_health:
            ...
            self.log(f"{self.name} heals {target.name} for {actual_heal:.1f} health. (Cost: {cost:.1f} energy)")
        else:
            ...
            self.divine_energy -= cost
            self.log(f"{self.name} grants mana to {target.name}: +{granted:.1f} MP (Cost: {cost:.1f} energy)")
